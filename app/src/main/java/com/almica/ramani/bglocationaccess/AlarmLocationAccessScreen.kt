...
import com.almica.ramani.locationupdates.PermissionBox

private const val logtag = "AlarmLocationAccessScreen"

/**
 * NOT working 04okt2025
 */
@RequiresApi(Build.VERSION_CODES.S)
@Composable
fun AlarmLocationAccessScreen(finish: () -> Unit) {
...
@Composable
private fun BackgroundLocationControls(finish: () -> Unit) {
    ...
    val alarmState = alarmSchedulerImpl.alarmManager.nextAlarmClock
    val controlsState = remember(alarmState) {
        // Find if there is any enqueued or running worker and provide UI state
        val enqueued = alarmState.isNotNull() //workerState?.find { !it.state.isFinished } != null
        if (enqueued) {
            ControlsState(
...
/*
                    workManager.enqueueUniquePeriodicWork(
                        BgLocationWorker.Companion.workName,
