...
internal fun calculateDistance(
    ...
    val canvasWidthMeters = SphericalUtil.computeDistanceBetween(upperLeftLatLng, upperRightLatLng)
...
fun ScaleBarPreview() {
    RamaniTheme {
        ScaleBar(cameraPositionState = rememberCameraPositionState())
    }
}

@Preview(showBackground = true)
@Composable
fun DisappearingScaleBarPreview() {
    RamaniTheme {
        DisappearingScaleBar(cameraPositionState = rememberCameraPositionState())
    }
}
