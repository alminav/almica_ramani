 ...
 */
data class DataPointWithDist(val x: Float, val y: Float, val geoLocation: LatLng?, val time: Long, var distKm : Double,
                             val description: String = "Distance of point is ${(1000 * distKm).formatDistM(true)}")
//data class DataPoint(val x: Float, val y: Float, val geoLocation: LatLng?, val distKm: Float,
//                     val srtmValue: Float, val description: String = "Distance of point is ${formatDistM(
//        (1000 * distKm).toDouble(), true)}", val time: Long) {
//}
...
data class LinePlot(
     ...
     */
    data class Line(
        val dataPoints: List<DataPointWithDist>,
    ...
    data class Intersection(
        ...
        val colorFilter: ColorFilter? = null,
        val blendMode: BlendMode = DrawScope.DefaultBlendMode,
        val draw: DrawScope.(Offset, DataPointWithDist) -> Unit = { center, _ ->
            drawCircle(
                color,
                radius.toPx(),
                center,
